...
public class FoodSource {
    ...
    public FoodSource(float maxRadius, float spawnRate, float foodLimit, Vector2f position) {
    ...
    public void draw() {
        ...
        glColor3f(1, 0, 0);
        glRectf(mPosition.x - SIZE / 2, mPosition.y + SIZE / 2, mPosition.x, mPosition.y);

        glColor3f(0, 1, 0);
        glRectf(mPosition.x, mPosition.y + SIZE / 2, mPosition.x + SIZE / 2, mPosition.y);

        glColor3f(0, 0, 1);
        glRectf(mPosition.x - SIZE / 2, mPosition.y - SIZE / 2, mPosition.x, mPosition.y);

        glColor3f(0.5f, 0.5f, 0.5f);
        glRectf(mPosition.x, mPosition.y, mPosition.x + SIZE / 2, mPosition.y - SIZE / 2);
    ...
    public void update(double delta) {
        ...
        if (framesAlive % mSpawnRate == 0) {
            ...
            speck.setPosition(new Vector2f(mPosition.x + RandomUtil.rand.nextFloat() * 2 * mMaxRadius - mMaxRadius,
                    mPosition.y + RandomUtil.rand.nextFloat() * 2 * mMaxRadius - mMaxRadius));
